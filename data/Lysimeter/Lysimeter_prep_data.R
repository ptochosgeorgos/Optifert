...
raw_list <- list.files("data/Lysimeter/raw_data/", pattern="*.csv", full.names = TRUE)

walk(raw_list, function(f) {
  obj_name <- str_remove_all(basename(f), c("Lysimeter_|\\.csv"))
  assign(obj_name, read_delim(f, show_col_types = FALSE), envir = .GlobalEnv)
})
...
saveRDS(
  Seepage_water_FIA2,
  "data/Lysimeter/prep_data/leachate_postsummer25.rds"
)
